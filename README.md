...
* **ImageUrl**: The SAS token authenticated URL of the baked Windows Image generated by `Publish-ClusterImage`.  The template should use this parameter to create a managed image resource.  
  *Must be supported if and only if an image is created with `Publish-ClusterImage` in the Service, Flighting Ring, or Environment.*
